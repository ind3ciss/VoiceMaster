...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('unlock')
    .setDescription('Déverrouille votre salon vocal privé'),
  ...
  async execute(interaction) {
    ...
    const confirmEmbed = new EmbedBuilder()
      .setColor('#fffc00')
      .setDescription('`🔓` Le salon vocal a été **déverrouillé**. Tout le monde peut le rejoindre.');
    await interaction.reply({ embeds: [confirmEmbed], ephemeral: false });

    const logEmbed = new EmbedBuilder()
      .setColor('#fffc00')
      .setDescription(`\`🔓\` \`${member.user.tag}\` a déverrouillé le salon **\`${userVoice.name}\`**`)
